            ...
            if($event->id && $event->type == 'payment_intent.succeeded') {
                if(isset($event->data['object']['metadata']['invoice_id'])){
                    ...
                    if($invoice) {
                        ...
                        $invoice->tax = $tax;
                        $invoice->save();
                        $invoice->user->updateBalance($invoice->amount_paid - $tax, 'Payment received for invoice #'.$invoice->invoice_id);
                    }
                    DB::commit();
                }
            }
    ...
    function stripeLinkPayment(Request $event) {
        try {
            if($event->id && $event->type == 'checkout.session.completed' && $event->livemode == true) {
                ...
                $invoice->tax = $tax;
                $invoice->save();
                $invoice->user->updateBalance($invoice->amount_paid - $tax, 'Payment received for invoice #'.$invoice->id);
                DB::commit();
            }
        } catch (\Exeception $th) {
            DB::rollBack();
